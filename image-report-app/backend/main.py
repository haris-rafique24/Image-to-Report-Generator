...
_cors_origins = [o.strip() for o in _extra.split(",") if o.strip()] + _default_origins
...
async def analyze_image(
    ...
    if client is None:
        raise HTTPException(
            status_code=503,
            detail="Server is not configured with GEMINI_API_KEY. Set it in environment variables.",
        )
